package com.teethen.sdk.xbottom.navigation;
...
import static com.teethen.sdk.xbottom.navigation.utils.Utils.getDrawable;
...
public class TextBadgeItem extends BadgeItem<TextBadgeItem> {
    ...
    private int mUnreadNumThreshold = 99;
     ...
     * @return
     */
    public TextBadgeItem setText(int num) {
        String text = String.valueOf(num);
        this.mText = text;
        if (isWeakReferenceValid()) {
            TextView tv = getTextView().get();
            if (!TextUtils.isEmpty(text)) {
                tv.setVisibility(num > 0 ? View.VISIBLE : View.GONE);
                if (num > mUnreadNumThreshold) {
                    tv.setText(String.format(Locale.CHINA, "%d+", mUnreadNumThreshold));
                } else {
                    tv.setText(text);
                }
            }
    ...
    /**
     * show text like 'new'. added by xingq 20180223
     * @param text
     * @return
     */
    public TextBadgeItem setMessage(@Nullable CharSequence text) {
        this.mText = text;
        if (isWeakReferenceValid()) {
            TextView tv = getTextView().get();
            if (!TextUtils.isEmpty(text)) {
                FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                params.setMargins(14, 14, 0, 0);
                params.gravity = Gravity.RIGHT | Gravity.TOP | Gravity.CENTER;
                tv.setLayoutParams(params);
                tv.setVisibility(View.VISIBLE);
                tv.setGravity(Gravity.CENTER);
                tv.setBackground(getDrawable(tv.getContext(), R.drawable.bottom_bar_shape_unread));
                tv.setTextSize(10);
                tv.setText(text);
            }
        }
