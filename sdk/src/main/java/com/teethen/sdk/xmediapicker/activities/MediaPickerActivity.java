...
import com.teethen.sdk.xmediapicker.utils.Utils;
import com.teethen.sdk.xmediapicker.CropListener;
import com.teethen.sdk.xmediapicker.MediaItem;
import com.teethen.sdk.xmediapicker.MediaOptions;
import com.teethen.sdk.xmediapicker.MediaSelectedListener;
import com.teethen.sdk.R;
import com.teethen.sdk.xmediapicker.imageloader.MediaImageLoader;
import com.teethen.sdk.xmediapicker.imageloader.MediaImageLoaderImpl;
import com.teethen.sdk.xmediapicker.utils.MediaUtils;
import com.teethen.sdk.xmediapicker.utils.MessageUtils;
import com.teethen.sdk.xmediapicker.utils.RecursiveFileObserver;
...
public class MediaPickerActivity extends AppCompatActivity implements
    ...
    //当用户点击'完成'菜单或者按下返回键或者物理返回键时的处理
    private void processSelectedMediaItems() {
        Fragment activePage;
        ...
        if (isPhoto) {
            if (mMediaOptions.isCropped() && !mMediaOptions.canSelectMultiPhoto()) {
                ...
                MediaItem mediaItem = new MediaItem(MediaItem.PHOTO, ((MediaPickerFragment) activePage).getMediaSelectedList().get(0).getUriOrigin());
    ...
    private void syncIconMenu(int mediaType) {
        switch (mediaType) {
            case MediaItem.PHOTO:
                mMediaSwitcher.setIcon(R.drawable.ic_play_circle_filled_white);
                mMediaSwitcher.setTitle(getString(R.string.picker_switch_video));
                break;
            case MediaItem.VIDEO:
                mMediaSwitcher.setIcon(R.drawable.ic_crop_original_white);
                mMediaSwitcher.setTitle(getString(R.string.picker_switch_image));
                break;
            default:
                mMediaSwitcher.setTitle(getString(R.string.picker_switch_media));
                break;
        }
    }
        ...
        if (takePictureIntent.resolveActivity(getPackageManager()) != null) {
            ...
            if (file != null) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    uri = FileProvider.getUriForFile(getApplicationContext(), Utils.FILE_PROVIDER, file);
                }
        ...
        if (takeVideoIntent.resolveActivity(getPackageManager()) != null) {
            ...
            if (max != Integer.MAX_VALUE) {
                ...
                if (mMediaOptions.isShowWarningVideoDuration()) {
                    MediaPickerErrorDialog dialog = MediaPickerErrorDialog.newInstance(MessageUtils.getWarningMessageVideoDuration(getApplicationContext(), max));
    ...
    private void tryCorrectPhotoFileCaptured() {
        if (mPhotoFileCapture == null || mFilesCreatedWhileCapturePhoto == null || mFilesCreatedWhileCapturePhoto.size() <= 0)
            return;
        ...
        for (File file : mFilesCreatedWhileCapturePhoto) {
            if (MediaUtils.isImageExtension(MediaUtils.getFileExtension(file))
                ...
                mPhotoFileCapture = file;
                Log.i(TAG, String.format("Try correct photo file: Delete duplicate photos in [%s] [%s]", mPhotoFileCapture, value));
                return;
            }
        }
    }
            ...
            switch (requestCode) {
                case REQUEST_PHOTO_CAPTURE:
                    ...
                    if (mPhotoFileCapture != null) {
                        ...
                        Uri uri = Uri.fromFile(mPhotoFileCapture);
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            uri = FileProvider.getUriForFile(getApplicationContext(), Utils.FILE_PROVIDER, mPhotoFileCapture);
                        }

                        if (mMediaOptions.isCropped()) {
                            MediaItem item = new MediaItem(MediaItem.PHOTO, uri);
