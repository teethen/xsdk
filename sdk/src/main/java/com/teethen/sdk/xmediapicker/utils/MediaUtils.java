...
public class MediaUtils {
    ...
    public static int getLastImageId(Context context) {
        ...
        Cursor cursor = context.getContentResolver().query(Images.Media.EXTERNAL_CONTENT_URI, imageColumns,null, null, imageOrderBy);
        if (cursor == null)
            return 0;
        int id = 0;
    ...
    public static long getDuration(Context context, Uri uri) {
        long duration = 0L;
        Cursor cursor = Video.query(context.getContentResolver(), uri, new String[]{Video.VideoColumns.DURATION});
        if (cursor != null) {
            cursor.moveToFirst();
            duration = cursor.getLong(cursor.getColumnIndex(Video.VideoColumns.DURATION));
            cursor.close();
        }
        return duration;
    }
