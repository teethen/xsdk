...
/**
 * Created by xingq on 2017/8/19.
 * 签名数据
 */

public class SignUtil {

    public static String url(String method, String json, String... signMethod) {
        String sign_method = "md5";
        if (signMethod != null && signMethod.length > 0) {
            sign_method = signMethod[0];
        }
        String tmpUrl = XConstant.SERVER_URL + method + "&sign_method=" + sign_method + "&timestamp=" + System.currentTimeMillis();
        String sign = sign(tmpUrl, json);
        String url = tmpUrl + "&sign=" + sign;
        return url;
    }

    private static String sign(String url, String body) {
        ...
        String secretKey = "84de41356e3e95928d025987b171f8e4";
    ...
    private Map<String, String> getParamsFromUrl(String url) {
        ...
        try {
            String fullUrl = URLDecoder.decode(url, XConstant.CHARSET_UTF8);
    ...
    private byte[] digest(String message)  {
        try {
            ...
            md5Instance.update(message.getBytes(XConstant.CHARSET_UTF8));
