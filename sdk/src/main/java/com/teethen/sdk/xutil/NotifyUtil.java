...
/**
 * Created by xingq on 2017/11/30.
 */
...
public class NotifyUtil {
    ...
    private void setCompatBuilder(PendingIntent pendingIntent, int smallIcon, String ticker, String title, String content, boolean sound, boolean vibrate, boolean lights) {
        ...
        //PendingIntent pIntent = PendingIntent.getActivity(mContext, requestCode, intent, FLAG);
        ...
        cBuilder.setFullScreenIntent(pendingIntent, true);
        /*
     ...
     * @param ticker
     */
    private void setCompatBuilder(PendingIntent pendingIntent, int largeIcon, int smallIcon, String ticker, String title, String content, boolean sound, boolean vibrate, boolean lights) {
        Bitmap bitmapImg = null;
        if (largeIcon > 0) {
            bitmapImg = BitmapFactory.decodeResource(mContext.getResources(), largeIcon);
        }
        cBuilder.setContentIntent(pendingIntent);// 该通知要启动的Intent
        cBuilder.setSmallIcon(smallIcon);// 设置顶部状态栏的小图标
        cBuilder.setLargeIcon(bitmapImg);// 设置顶部状态栏的大图标
        cBuilder.setTicker(ticker);// 在顶部状态栏中的提示信息
        cBuilder.setContentTitle(title);// 设置通知中心的标题
        cBuilder.setContentText(content);// 设置通知中心中的内容
        cBuilder.setWhen(System.currentTimeMillis());
        cBuilder.setAutoCancel(true);
        cBuilder.setPriority(NotificationCompat.PRIORITY_MAX + 1);
        cBuilder.setFullScreenIntent(pendingIntent, true);
     ...
     * @param content
     */
    public void notifyNormalSingleLine(PendingIntent pendingIntent, int largeIcon, int smallIcon, String ticker, String title, String content, boolean sound, boolean vibrate, boolean lights) {
        setCompatBuilder(pendingIntent, largeIcon, smallIcon, ticker, title, content, sound, vibrate, lights);
        sent();
    }
     ...
     * @param content
     */
    public void notifyNormalMoreLine(PendingIntent pendingIntent, int largeIcon, int smallIcon, String ticker, String title, String content, boolean sound, boolean vibrate, boolean lights) {
        final int sdk = Build.VERSION.SDK_INT;
        if (sdk < Build.VERSION_CODES.JELLY_BEAN) {
            notifyNormalSingleLine(pendingIntent, largeIcon, smallIcon, ticker, title, content, sound, vibrate, lights);
     ...
     * @param bigPic
     */
    public void notifyBigPic(PendingIntent pendingIntent, int smallIcon, String ticker, String title, String content,
    ...
    public void notifyHeadsUp(PendingIntent pendingIntent, int smallIcon, int largeIcon, String ticker, String title, String content,
                              int leftBtnIcon, String leftText, PendingIntent leftPendingIntent,
                              int rightBtnIcon, String rightText, PendingIntent rightPendingIntent,
    ...
    private void sent() {
        notification = cBuilder.build();
        nm.notify(NOTIFICATION_ID, notification);// 发送该通知
    }
